...
//                if(p.getName().equals("natzu no taizai")) throw new SQLException("Can't save natzu no taizai");
                ps.execute();
            } catch (SQLException e) {
                e.printStackTrace();
                shouldRollback = true;
    ...
    public static void update(Producer producer) {
        ...
        try (Connection conn = ConnectionFactory.getConnection();
             Statement stmt = conn.createStatement()) {
            int rows = stmt.executeUpdate(sql);
            log.info("Update '{}', Database rows affected '{}'", producer.getId(), rows);
        } catch (SQLException e) {
    ...
    public static void updatePreparedStatement(Producer producer) {
        try (Connection conn = ConnectionFactory.getConnection();
            ...
            log.info("Update prepared statement '{}', Database rows affected '{}'", producer.getId(), rows);
        } catch (SQLException e) {
            log.error("Error while trying to update producer '{}'", producer.getId(), e);
        }
    }
